...
struct BaseStrategyWithThreads : public Strategy {
  ...
  void StartNextRound() override {
    ...
    this->TerminateTasks(); // TODO: what about different threads count for wmm_graph?
    ...
    if (this->round_schedule.size() != this->threads_count) {
      this->round_schedule.resize(this->threads_count, -1);
    }
  }
  ...
  void SetCustomRound(CustomRound& custom_round) override {
    ...
    for (size_t current_thread = 0; current_thread < custom_threads_count;
      ...
      for (auto& task_builder : builders) {
        auto task = task_builder.Build(state.get(), current_thread, new_task_id++);
...
struct StrategyScheduler : public SchedulerWithReplay {
  ...
  Scheduler::Result Run() override {
    for (size_t j = 0; j < custom_rounds.size() + max_rounds; ++j) {
      ...
      if (is_running_custom_scenarios) {
        ...
        debug(stderr, "explore custom round: %d\n", i);
 ...
 protected:
  ...
  Result GenerateAndRunRound() override {
...
struct TLAScheduler : Scheduler {
 ...
 private:
  ...
  struct Frame {
    ...
    Task* task{};
    // Id of thread in which task is running.
    int thread_id;
  ...
  std::tuple<bool, typename Scheduler::Result> RunStep(size_t step,
    ...
    for (size_t thread_id = 0; thread_id < threads.size(); ++thread_id) {
      ...
      for (size_t cons_num = 0; auto cons : constructors) {
        ...
        tasks.emplace_back(cons.Build(&state, thread_id, -1 /* TODO: fix task id for tla, because it is Scheduler and not Strategy class for some reason */));
        ...
        tasks.back()->Terminate(thread_id);
        tasks.pop_back();
